...
- **Dark-only for now.** `<html className="dark">` in the root layout is hardcoded; the light-mode token values in `:root` are kept current (not stale) so removing that class is the entire scope of a future light mode — don't let them drift out of sync with `.dark`.
